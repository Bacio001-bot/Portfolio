            ...
            <div class=" relative w-[10rem] ml-5">
                ...
                <div x-cloak x-show="open" @click.outside="open = false">
                    <ul class="absolute z-10 mt-1 w-full bg-white shadow-lg max-h-60 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm"
                        ...
                        <li wire:click="sortCategory('firstname')"
                            class="hover:bg-gray-100 cursor-pointer text-gray-900 select-none relative py-2 pl-3 pr-9"
                            id="listbox-option-0" role="option">
                            <span class="font-normal block truncate"> Firstname </span>
                        </li>
                        <li wire:click="sortCategory('infix')"
                            class="hover:bg-gray-100 cursor-pointer text-gray-900 select-none relative py-2 pl-3 pr-9"
                            id="listbox-option-0" role="option">
                            <span class="font-normal block truncate"> infix </span>
                        </li>
                        <li wire:click="sortCategory('lastname')"
                            class="hover:bg-gray-100 cursor-pointer text-gray-900 select-none relative py-2 pl-3 pr-9"
                            id="listbox-option-0" role="option">
                            <span class="font-normal block truncate"> Lastname </span>
                        </li>
                        ...
                        <li wire:click="sortCategory('phonenumber')"
                            class="hover:bg-gray-100 cursor-pointer text-gray-900 select-none relative py-2 pl-3 pr-9"
                            id="listbox-option-0" role="option">
                            <span class="font-normal block truncate"> Phonenumber </span>
                        </li>
                    </ul>
                </div>
    ...
    <div class="overflow-x-auto shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
        <table class="min-w-full divide-y divide-gray-300">
            <thead class="bg-gray-50">
                <tr>
                    <th wire:click="sortBy('firstname')" scope="col" class="cursor-pointer py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-gray-900 sm:pl-6">Firstname</th>
                    <th wire:click="sortBy('infix')" scope="col" class="cursor-pointer px-3 py-3.5 text-left text-sm font-semibold text-gray-900">Infix</th>
                    <th wire:click="sortBy('lastname')" scope="col" class="cursor-pointer px-3 py-3.5 text-left text-sm font-semibold text-gray-900">Lastname</th>
                    <th wire:click="sortBy('company_id')" scope="col" class="cursor-pointer px-3 py-3.5 text-left text-sm font-semibold text-gray-900">Company</th>
                    ...
                    <th wire:click="sortBy('phonenumber')" scope="col" class="cursor-pointer px-3 py-3.5 text-left text-sm font-semibold text-gray-900">Phonenumber</th>
            ...
            <tbody class="divide-y divide-gray-200 bg-white">
                ...
                <tr x-data="{ menu:false }">
                    <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6">{{$employee->firstname}}</td>
                    <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{$employee->infix}}</td>
                    ...
                    <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{$employee->email}}   
                    <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{$employee->phonenumber}}
                        <ul x-cloak x-show="menu" x-on:click.outside="menu = false"  class="w-[10rem] right-10 cursor-pointer text-center absolute px-2 bg-white border rounded-lg">
                            <li class="py-1 border-b" onclick="return confirm('Are you sure?') ? @this.delete({{$employee->id}}) : false" >Delete</li>
